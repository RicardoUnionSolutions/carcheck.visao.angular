...
export class TipoProdutoPacotesComponent implements OnInit {
  ...
  setValores() {
    this.pacotes.forEach((pacote, index) => {
      ...
      const composta = this.consultas.find(consulta => consulta.id === idComposta);
      const valorAtual = composta.valor_promocional * pacote.quantidade_composta;
      const valorPromocional = valorAtual * (1 - pacote.porcentagem_desconto / 100);
