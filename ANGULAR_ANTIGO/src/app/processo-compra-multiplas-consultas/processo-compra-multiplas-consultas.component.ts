...
export class ProcessoCompraMultiplasConsultasComponent implements OnInit {
  ...
  dadosVeiculo: any = { anoFabricacao: '', anoModelo: '', modelo: '', placa: '', chassi: '', corVeiculo: '' };
  ...
  constructor(
    ...
    this.consultas = [
      {
        composta: { id: 3 },
        label: 'Completa', icon: 'completa.png', valorConsulta: 54.90, valorReais: '', valorCentavos: '', quantidade: 0,
      ...
      {
        composta: { id: 5 },
        label: 'Segura', icon: 'segura.png', valorConsulta: 48.90, valorReais: '', valorCentavos: '', quantidade: 0,
      ...
      {
        composta: { id: 2 },
        label: 'Leilão', icon: 'leilao.png', valorConsulta: 32.90, valorReais: '', valorCentavos: '', quantidade: 0,
  ...
  ngOnInit() {
    this.titleService.setTitle('Comprar Pacotes de Consultas Veiculares - CarCheck');
    this.metaService.updateTag({
      name: 'description',
      content: 'Compre pacotes de consultas veiculares completas, seguras ou de leilão com facilidade e segurança. Ideal para quem precisa analisar múltiplos veículos.'
    });
  }
  ...
  next() {
    ...
    switch (this.currentStep) {
      case this.stepIndex.consulta:
        this.nextStep();
        this.analyticsService.addToCartCredito(this.consultas.filter(c => c.quantidade > 0));
  ...
  stepPagamentoEvent() {
    ...
    let objPagamento = { dadosProprietario: dadosProprietario, consultasSelecionadas: consultasSelecionadas, pacote: pacoteSelecionado, pagamento: pagamento, dadosVeiculo: this.dadosVeiculo };
          ...
          this.pagamentoService.pagar(objPagamento)
            .then(rs => {
              ...
              switch (rs.situacaoPagamento) {
                ...
                case 'APROVADO':
                  this.pagamentoService.setUltimaCompra(rs, consultasSelecionadas, 'multipla');
  ...
  formataMesagemErro(erro: string) {
    let lErro = [];
    if (erro != null && erro != undefined && erro.replace(" ", "") != "" && erro.length > 0) {
      ...
      lErro.push('Ocorreu um erro no processamento do pagamento, verifique que seus dados estão corretos');
    }
    return lErro;
  }
