...
export class ProcessoCompraComponent implements OnInit, AfterViewInit {
  ...
  laudo = { chassi: '', placa: '', proprietario: '', telefone: '', uf: '', cidade: '' };
  laudoStatus = 'INVALID';
  ...
  constructor(
    ...
    this.loginService.getLogIn().subscribe(v => {
      this.logIn = v;
      if (v.status == true && v.cliente.documento == '' || v.cliente.documento == '000.000.000-00' || v.cliente.documento == '00.000.000/0000-00') {
        this.cadastrar = true;
        ...
        this.cadastrarUsuario.clienteTipoIndefinido = this.logIn.cliente.clienteTipoIndefinido;
        this.cadastrarUsuario.tipoPessoa = this.logIn.cliente.tipoPessoa == 'FISICA' ? '0' : '1';
        this.cadastrarUsuario.tokenFacebook = this.logIn.tokenFacebook != null ? this.logIn.tokenFacebook : '';
      }

    });

    this.loginService.getLogIn().pipe(take(1)).subscribe(v => {
      ...
      if (this.route.snapshot.paramMap.get('consulta') != null && !this.validaPlaca(this.route.snapshot.paramMap.get('consulta'))) {
        this.nextStep();
      }

      // Verificação se o primeiro parametro é placa
      if (this.validaPlaca(this.route.snapshot.paramMap.get('consulta'))) {
        this.pesquisaPlaca = (this.route.snapshot.paramMap.get('consulta')).toUpperCase().replace("-", "");
      }
  ...
  ngOnInit() {
    ...
    this.meta.updateTag({
      name: 'description',
      content: 'Realize sua compra de consulta veicular em etapas simples: escolha, cadastro e pagamento seguro.'
    });
    //  this.verificaCompraAprovada();

    if (this.pesquisaPlaca == "") {
      this.pesquisaPlaca = (this.route.snapshot.paramMap.get('placa') || '').toUpperCase();
    }
    this.pesquisaEmail = (this.route.snapshot.paramMap.get('email') || '').toUpperCase();
  }
  ...
  stepVeiculoEvent() {
    ...
    if (this.tipoPesquisa == 'chassi' && this.pesquisaChassi != '') {
      ...
      this.fazerConsultaService.consultar(this.pesquisaChassi)
        .subscribe(
          dados => {
            if (dados == null) {
    ...
    if (this.tipoPesquisa == 'placa' && this.pesquisaPlaca != '') {
      this.fazerConsultaService.consultar(this.pesquisaPlaca)
        .subscribe(
          dados => {
            this.dadosVeiculo = dados;
  ...
  stepCadastroEvent() {
    if (this.cadastrar == false && this.logIn.cliente.documento != '000.000.000-00' && this.logIn.cliente.documento != '00.000.000/0000-00') {
      this.currentStep = 1;
      this.escondeCadastro();
      this.nextStep();
      return;
    ...
    if ((this.logIn.cliente.documento == '000.000.000-00' || this.logIn.cliente.documento == '00.000.000/0000-00')) {
      this.nextStep();
      return;
    }
    ...
    if (this.cadastrarUsuario.formAcesso['valid']) {
      ...
      if (delete cadastro.formCadastro != null)
      ...
      this.pessoaService.adicionar(cadastro)
        .then(response => {
          this.loadingCompra = false;
          this.modalService.closeLoading();
          if (response == 'erro_email') {
            this.cadastrarUsuario.formAcesso['controls']['email'].setErrors({ msg: 'E-mail já cadastrado' });
            return;
          }
  ...
  stepPagamentoEvent() {
    ...
    let dadosProprietario = { nomeProprietario: this.laudo.proprietario, telefone: this.laudo.telefone, uf: this.laudo.uf, cidade: this.laudo.cidade };
    let objPagamento = { dadosVeiculo: this.dadosVeiculo, consultasSelecionadas: this.consultasSelecionadas, pagamento: pagamento, dadosProprietario: dadosProprietario };
    this.analyticsService.registroEntrandoPagamento();

    this.modalService.openModalMsg({
      status: 1, title: 'Confirmação de compra',
      text: 'Tem certeza que deseja realizar consulta ' + this.consultasSelecionadas[0].composta.nome.toLowerCase() + ' para ' +
        (this.tipoPesquisa == 'placa' ? ' a placa ' + this.pesquisaPlaca.toLocaleUpperCase() : ' o chassi ' + this.pesquisaChassi.toLocaleUpperCase()) + '?',
      ok: {
        event: () => {
          this.loadingCompra = true;
          this.pagamentoService.pagar(objPagamento)
            .then(rs => {
              this.loadingCompra = false;
              ...
              switch (rs.situacaoPagamento) {
                ...
                case 'APROVADO':
                  this.pagamentoService.setUltimaCompra(rs, this.consultasSelecionadas, 'unica');
  ...
  next() {
    ...
    switch (this.currentStep) {
      ...
      case this.stepIndex.veiculo:
        ...
        this.analyticsService.addToCart(this.consultasSelecionadas[0].composta.nome);
  ...
  nextStep() {
    if (this.currentStep < this.steps.length - 1) {
      this.currentStep++;
      this.doneSteps = this.currentStep > this.doneSteps ? this.currentStep : this.doneSteps;
      this.msg = '';
    }
  }
  ...
  async carregaOpcoesConsultaUsuario() {
    try {
      ...
      this.listaConsulta = await this.fazerConsultaService.getConsultaCliente(param).toPromise();
  ...
  selecionarTipoConsulta() {
    const nomeConsulta = (this.route.snapshot.paramMap.get('consulta') || '').toLowerCase();
    this.listaConsulta.forEach(c => c.quantidade = 1);
    let consultaSelecionada = this.listaConsulta.find(c => c.composta.nome.toLowerCase().includes(nomeConsulta));
    consultaSelecionada = consultaSelecionada || this.listaConsulta.find(c => c.principal)
    consultaSelecionada = consultaSelecionada || this.listaConsulta[0] || null;

    this.consultasSelecionadas = [consultaSelecionada];
    this.consultaLaudo = this.listaConsulta.find(c => c.composta.tipoProduto == 'LAUDO');
    this.possuiLaudo = consultaSelecionada.composta.tipoProduto == 'LAUDO';
    const next = (this.route.snapshot.queryParamMap.get('next') || '').toLocaleUpperCase();
  ...
  calcularValorTotal() {
    this.valorTotal = this.consultasSelecionadas.reduce((a, b) => a + b.valorConsulta, 0);
  ...
  toggleLaudoAdicional($e) {
    ...
    if (this.laudoAdicional) {
      ...
      this.consultasSelecionadas = [...this.consultasSelecionadas, this.consultaLaudo];
    } else {
      this.resetLaudo();
      this.consultasSelecionadas = this.consultasSelecionadas.filter(c => c != this.consultaLaudo);
    }
    this.calcularValorTotal();
  }
  ...
  resetLaudo() {
    this.laudo = { chassi: '', placa: '', proprietario: '', telefone: '', uf: '', cidade: '' };
  }
