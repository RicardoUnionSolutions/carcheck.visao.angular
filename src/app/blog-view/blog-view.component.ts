...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, PaginationModule, AppearOnScrollDirective, AppearRightOnScrollDirective],
})
export class BlogViewComponent implements OnInit {
  ...
  async loadPosts() {
    try {
      ...
      if (category || this.categoriaSlug) {
        this.categoriaSlug != ""
          ? (category = this.categoriaSlug)
          : (category = category);
        const postagensByCategoria =
          await this.postService.getPostsByCategorias(
            category,
            this.currentPage,
            this.itemsPerPage
          );
  ...
  async getPostsByCategory(categoriaSlug, categoria) {
    ...
    this.itemsPerPage = 4;
    try {
      const postagensByCategoria = await this.postService.getPostsByCategorias(
        categoriaSlug,
        this.currentPage,
        this.itemsPerPage
      );
