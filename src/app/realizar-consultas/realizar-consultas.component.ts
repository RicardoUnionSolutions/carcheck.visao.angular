...
export class RealizarConsultasComponent implements OnInit {
  ...
  carregaDadosVeiculoConsulta(i, concat: String = "") {
    var dados =
      this.consultas[i].tipoPesquisa == "placa"
        ? this.consultas[i].placa.toUpperCase()
        : this.consultas[i].chassi.toUpperCase();
    ...
    this.fazerConsultaService.consultar(dados).subscribe(
      (data: any) => {
        this.dadosVeiculo = data;
        if (data != null) {
          concat +=
            "<br>" +
            this.consultas[i].consultaSelecionada.nome +
            " - " +
            (this.consultas[i].tipoPesquisa == "placa"
              ? " Placa: " + this.consultas[i].placa.toUpperCase()
              : " Chassi: " + this.consultas[i].chassi.toUpperCase()) +
            " - " +
            data.marca +
            "/" +
            data.modelo;
  ...
  carregarDadosCreditoConsulta() {
    this.modalService.openLoading({
      title: "Carregando...",
      text: "Aguarde, carregando consultas disponíveis.",
    });
    this.dadosConsultaService.getCreditoConsulta();

    const credito$ = this.dadosConsultaService.getCreditoConsulta?.();
    if (!credito$ || typeof (credito$ as any).subscribe !== "function") {
      this.modalService.closeLoading();
      return;
    }
    credito$.subscribe(
      (data: any) => {
        ...
        for (let d of data) {
          if (d.quantidade > 0)
            options.push({
              label: d.nome,
              value: { ...d, quantidadeSelecionada: 0 },
            });
        }
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle(
      "Realizar Consulta de Veículos por Placa ou Chassi"
    );
    this.metaService.updateTag({
      name: "description",
      content:
        "Página para realizar consultas veiculares detalhadas usando placa ou chassi, com múltiplas opções de crédito.",
    });
  }
  ...
  verificarDisponibilidadeConsulta() {
    ...
    for (let i = 0; i < this.consultasOptions.length; i++) {
      if (
        this.consultasOptions[i].value.quantidade ==
        this.consultasOptions[i].value.quantidadeSelecionada
      )
        indisponiveis++;
    }
  ...
  consultar() {
    ...
    for (let i = 0; i < this.consultas.length; i++) {
      if (
        this.consultas[i].tipoPesquisa == "placa" &&
        this.consultas[i].placa.length < 7
      ) {
      ...
      if (
        this.consultas[i].tipoPesquisa == "chassi" &&
        this.consultas[i].chassi.length < 17
      ) {
        //console.log(this.consultas[i]);
    ...
    this.modalService.openModalMsg({
      ...
      ok: {
        event: () => {
          setTimeout(() => {
            this.modalService.openLoading({
              title: "Efetuando consulta",
              text: "Aguarde, sua consulta está sendo realizada.",
            });
            this.fazerConsultaService.realizarConsulta(consultas).subscribe(
              () => {
                this.modalService.closeLoading();
                ...
                this.notificationService.addNotification(
                  "Consulta realizada",
                  "Sua consulta esta pronta para ser vizualizada"
                );
              },
  ...
  updateConsultasOptions() {
    this.consultas.forEach((c) => {
      ...
      this.consultasOptions.forEach((o) => {
        if (
          o.value.quantidade > o.value.quantidadeSelecionada ||
          (c.consultaSelecionada != null &&
            o.value.nome == c.consultaSelecionada.nome)
        )
          c.options.push(o);
      });
    });
